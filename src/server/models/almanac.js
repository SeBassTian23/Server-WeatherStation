...
const airqualityToStr = function( data, type, units ){
    ...
    for( phrase in airquality ){
        if( data.max !== null && data.max <= airquality[phrase][1] ){
            if(  str != avg && type == "day")
                str += ` reaching <strong>${phrase}</strong> levels during the ${timeToStr(data.max_time, type)}`;
...
const almanacSummary = function( data, type, units ){
