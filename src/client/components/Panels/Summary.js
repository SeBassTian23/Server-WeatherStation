...
function SummaryTable(props) {
  ...
  useEffect(() => {
    if (left.length > 0 || right.length > 0)
      setDownloadCSVLink(downloadLink(props.period, props.selectedDate, state.units) || null);
    else
      setDownloadCSVLink(null);
  }, [props.selectedDate])

  return (
    <Card>
      <Card.Body>
        {downloadCSVLink && <a className='float-end me-2' href={downloadCSVLink} aria-label="Download Latest Observations as CSV"><i className='bi bi-download text-muted' /></a>}
        <Card.Title className='text-info'>Summary</Card.Title>
